...
### 1. Останови и удали все контейнеры + volumes
sudo docker compose down -v

### 2. Удали старый образ
sudo docker rmi custom-airflow:slim-2.8.1-python3.11

### 3. (Опционально) Если хочешь почистить всё неиспользуемое
sudo docker system prune -f

### 4. Собери образ заново (--no-cache гарантирует чистую сборку)
sudo docker build --no-cache -t custom-airflow:slim-2.8.1-python3.11 .
