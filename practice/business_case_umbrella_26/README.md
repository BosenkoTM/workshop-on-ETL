...
- Получить прогноз погоды в **Москве на 7 дней** (используя внешний API).
- Обработать данные: добавить столбец **"день недели"**.
- Сгенерировать моковые данные о продажах за эти же даты и объединить наборы данных.
- Обучить простейшую ML-модель (Линейная регрессия).
- Построить **график температуры по дням недели** (в качестве инструмента визуализации добавлен Streamlit).
