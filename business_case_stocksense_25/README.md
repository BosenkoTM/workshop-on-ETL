...
Ниже приведено объяснение в разметке GitHub о том, как подключиться к базе данных PostgreSQL через DBeaver для просмотра результатов в таблице, используя информацию из вашего конфигурационного файла Docker:
...
1. **Убедитесь, что контейнер PostgreSQL работает**
   ...
   ```bash
   sudo docker-compose up -d wiki_results
   ```
